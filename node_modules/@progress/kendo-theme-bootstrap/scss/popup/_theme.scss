@import "../../modules/@progress/kendo-theme-default/scss/popup/_theme.scss";

@include exports( "popup/theme/bootstrap" ) {

    .k-popup {

        .k-header,
        .k-group-header {
            color: $dropdown-header-color;
        }

        .k-outer-group-header {
            border-top-color: $grouping-header-border;
            color: $frozen-grouping-header-text;

            &.k-first {
                border-color: $grouping-header-border;
            }
        }
    }

    .k-popup > .k-group-header,
    .k-popup > .k-virtual-wrap > .k-group-header {
        border-bottom-color: $grouping-header-border;
    }

    .k-popup > .k-group-header,
    .k-popup .k-outer-group-header.k-first,
    .k-popup > .k-virtual-wrap > .k-group-header {
        box-shadow: none;
    }

}

@include exports( "list/theme/bootstrap" ) {

    .k-list {}

    .k-list .k-item:focus,
    .k-list .k-item.k-state-focused,
    .k-list-optionlabel:focus,
    .k-list-optionlabel.k-state-focused {
        box-shadow: inset 0 0 0 3px rgba($list-item-text, .15);

        &:hover,
        &.k-state-hover {
            box-shadow: inset 0 0 0 3px rgba($list-item-hovered-text, .15);
        }

        &.k-state-selected {
            box-shadow: inset 0 0 0 3px rgba($list-item-selected-text, .15);
        }
    }

    .k-animation-container-shown,
    .k-animation-container > .k-popup {
        @include box-shadow( none );
    }

}


@include exports( "listcontainer/theme/bootstrap" ) {


    .k-list-container {

        // Button normal state
        .k-button {}

        // Button focus state
        .k-button:focus,
        .k-button.k-state-focused {
            box-shadow: inset 0 0 0 3px rgba($list-item-text, .15);

            &:hover,
            &.k-state-hover {
                box-shadow: inset 0 0 0 3px rgba($list-item-hovered-text, .15);
            }

            &:active,
            &.k-state-active {
                box-shadow: inset 0 0 0 3px rgba($list-item-selected-text, .15);
            }
        }

    }

}
